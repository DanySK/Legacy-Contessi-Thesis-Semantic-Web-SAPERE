...
import it.apice.sapere.node.networking.guestsmngt.GuestMessage;
import it.apice.sapere.node.networking.guestsmngt.GuestMessageType;
...
public final class CommunicationUtils {
	...
	public static boolean sendMessage(final Message mess, final Socket socket) {
		try {
			...
			LogFactory.getLog(CommunicationUtils.class).warn(
					"Cannot send the message", e);
		}
		return false;
	}

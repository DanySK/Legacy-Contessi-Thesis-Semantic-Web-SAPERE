...
import it.apice.sapere.node.networking.obsnotifications.Notification;
import it.apice.sapere.node.networking.obsnotifications.NotifierAgent;
import it.apice.sapere.node.networking.obsnotifications.SubscriptionRequest;
import it.apice.sapere.node.networking.obsnotifications.SubscriptionType;
...
public final class GuestsHandlerAgent extends AbstractSAPEREAgent {
	...
	public static GuestsHandlerAgent getInstance() {
		if (instance == null) {
			instance = new GuestsHandlerAgent("guests_agent");
		}
 ...
 * 
 */
class RequestHandler extends SystemAgent {
		...
		try {
			...
			if (socket != null) {
				try {
					...
					LogFactory.getLog(SendMessageThread.class).error(
							"Cannot close sending socket", e);
				}
			}
		}
